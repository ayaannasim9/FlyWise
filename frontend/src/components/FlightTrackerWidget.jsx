...
export default function FlightTrackerWidget({
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiBaseUrl}/trackFlight?${params.toString()}`
      );
  ...
  const cardClass =
    layout === "compact"
      ...
      : "bg-white/90 backdrop-blur rounded-3xl shadow-xl border border-white/60";

  return (
    <div className="space-y-8">
      <form
        onSubmit={handleSubmit}
        className={`${cardClass} p-6 lg:p-8 space-y-6`}
      >
